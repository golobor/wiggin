...
class SimConstructor:
    ...
    def configure(self):
        for order, action in sorted(self._actions, key=lambda x: x[0][0]):
            ...
            conf_res = action.configure(self.shared_config, self.action_configs)

            assert (
                conf_res is not None
        ...
        for order, action in sorted(self._actions, key=lambda x: x[0][1]):
            if hasattr(action, "run_init"):
                new_sim = action.run_init(
                    self.shared_config, self.action_configs, self._sim
                )
                if new_sim is None:
                    continue
                    ...
                    raise ValueError(
                        f"{action.name}.run_init() returned {new_sim}. "
            ...
            for order, action in sorted(self._actions, key=lambda x: x[0][2]):
                if hasattr(action, "run_loop"):
                    ...
                    if new_sim is None:
                        ...
                        raise ValueError(
                            f"{action.name}.run_loop() returned {new_sim}. "
                            "Allowed values are: polychrom.simulation.Simulation, None or False"
                        )
...
class BlockStep(SimAction):
    def __init__(self, num_blocks=100, block_size=int(1e4)):
...
class LocalEnergyMinimization(SimAction):
    def __init__(self, max_iterations=1000, tolerance=1, random_offset=0.1):
    ...
    def run_init(self, shared_config, action_configs, sim):
        ...
        if self_conf["repulsion_e"]:
            if self_conf["attraction_e"] and self_conf["attraction_r"]:
                ...
                nonbonded_force_kwargs = {"trunc": self_conf["repulsion_e"]}
...
class CrosslinkParallelChains(SimAction):
    ...
    def configure(self, shared_config, action_configs):
        ...
        if action_config["chains"] is None:
            action_config["chains"] = [
                (
                    (0, shared_config_added_data["N"] // 2, 1),
                    (
...
class GenerateRWInitialConformation(SimAction):
    ...
    def configure(self, shared_config, action_configs):
        ...
        ] = starting_conformations.create_random_walk(
            step_size=1.0, N=shared_config["N"]
        )
...
class AddGlobalVariableDynamics(SimAction):
    def __init__(
        self, variable_name=None, final_value=None, inital_block=0, final_block=None
    ):
        params = {
            k: v for k, v in locals().items() if k not in ["self"]
        }  # This line must be the first in the function.
